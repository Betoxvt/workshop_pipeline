...
# Function to load data from gold_vendas_por_vendedor_data
def carregar_dados_gold_vendas_por_vendedor_data():
    conn = conectar_db()
    query = "SELECT * FROM gold_vendas_por_vendedor_data;"
    with conn.cursor() as cursor:
        cursor.execute(query)
        dados = cursor.fetchall()
        colunas = [desc[0] for desc in cursor.description]
